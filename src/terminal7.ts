...
export class Terminal7 {
    ...
    constructor(
        ...
        this.logBuffer = new CyclicArray(
            settings.logLines || DEFAULT_LOG_LINES,
        );
        this.pendingPanes = {};
    ...
    async open() {
        ...
        document
            .getElementById("help-gate")
            .addEventListener("click", () => this.toggleHelp());
        document
            .getElementById("keys-help")
            .addEventListener("click", () => this.toggleHelp());
        document
        ...
        document
            .getElementById("peerbook-legend")
            .addEventListener("click", async (ev) => {
                setTimeout(
        ...
        document
            .getElementById("dotfile-button")
            .addEventListener("click", () =>
                this.map.shell.confEditor
        ...
        this.map.open().then(() => {
            ...
            setTimeout(async () => {
                ...
                if (runs % 12 == 11) RateApp.requestReview();
            }, 100);
        });
        this.initSearch();
    ...
    initSearch() {
        ...
        document
            .querySelector(".search-up")
            .addEventListener("click", () =>
                this.activeG.activeW.activeP.findPrev(),
            );

        document
            .querySelector(".search-down")
            .addEventListener("click", () =>
                this.activeG.activeW.activeP.findNext(),
            );
        //rename box
        document
            .querySelector(".rename-close")
            .addEventListener("click", () =>
        ...
        return new Promise((resolve, reject) => {
            ...
            Preferences.get({ key: "last_state" }).then(({ value }) => {
                if (!value) reject();
                    ...
                    if (!gate) {
                        console.log(
                            "Invalid restore state. Starting fresh",
                            state,
                        );
        ...
        return new Promise<void>((resolve, reject) => {
            ...
            if (this.pb) {
                ...
                this.getFingerprint().then((fp) => {
                    ...
                    this.pb.startPurchases().then(complete).catch(callReject);
                });
            }
        });
    }
    ...
    notify(message, dontShow = false) {
        ...
        if (!dontShow) this.map.showLog(true);
    }
    run(cb, delay) {
    ...
    loadConf(conf) {
        ...
        this.conf.ui.pinchMaxYVelocity =
            this.conf.ui.pinch_max_y_velocity || 0.1;
        this.conf.ui.autoRestore = this.conf.ui.auto_restore || false;
        this.conf.ui.verificationTTL =
            this.conf.ui.verification_ttl || 15 * 60 * 1000;
        this.conf.ui.subscribeTimeout =
            this.conf.ui.subscribe_timeout || 60 * 1000;
        ...
        this.conf.net.peerbook = this.conf.net.peerbook || DEFAULT_PB_HOST;
        ...
        this.conf.theme.selectionBackground =
            this.conf.theme.selection_background ||
            this.conf.theme.selectionBackground ||
            "#D9F505";
        this.conf.theme.selectionForeground =
            this.conf.theme.selection_foreground ||
            this.conf.theme.selectionForeground ||
            "#271D30";
        ...
        } else this.conf.peerbook = { insecure: false };
    }
    ...
    getFingerprint(): Promise<string> {
        ...
        this.fingerprintPromise = this._getFingerprint();
        // Clear the cached promise only after it settles, so that
    ...
    _getFingerprint(): Promise<string> {
                    ...
                    store
                        ...
                        .then((certificates) => {
                            if (certificates.length == 0) {
                                console.log(
                                    "got no certificates, generating",
                                    certificates,
                                );
                                this.generateCertificate()
                                    .then((cert) => resolve(compactCert(cert)))
                            ...
                            if (this.isCertificateExpired(cert)) {
                                this.log(
                                    "Stored certificate expired, regenerating",
                                );
                                this.notify(
                                    "Security certificate expired. Generating new certificate...",
                                );
            ...
            openDB("t7", 1, {
                ...
                .then((db) => {
                    const tx = db.transaction("certificates", "readwrite"),
                        store = tx.objectStore("certificates"),
                        c = this.certificates[0] as RTCCertificate & {
                            id: number;
                        };
                    c.id = 1;
                    store
    ...
    async toggleHelp(doShow: boolean = null) {
        ...
        */
        ...
        this.buttonHelpBubbles.forEach((bhb) => {
            if (
                (!isPaneShown &&
                    !bhb.parentElement?.classList.contains("off")) ||
                isPaneShown ||
                funcName === "add"
            )
    ...
    async showGreetings() {
        ...
        if (!greeted) {
            ...
            if (Capacitor.isNativePlatform()) this.map.tty(WELCOME_NATIVE);
            else this.map.tty(WELCOME_OTHER);
        } else {
            this.map.shell.printPrompt();
            if (
                !(
    ...
    clearTempGates() {
        this.gates.forEach((g) => {
            if (g.name.startsWith("temp_")) g.delete();
        });
    }
    ...
    resetGates() {
        if (this.activeG) this.activeG.close();
        this.gates.forEach((g) => {
            g.e.remove();
            this.map.remove(g);
    ...
    async getDotfile() {
        return (
            (await Preferences.get({ key: "dotfile" })).value || DEFAULT_DOTFILE
        );
    }
    saveDotfile(text) {
        ...
        this.cells.forEach((c) => {
            if (c instanceof Pane) c.setTheme(this.conf.theme);
        });
        // resize TWR
    ...
    async pbVerify() {
        ...
        try {
            response = await fetch(
                `${schema}://${this.conf.net.peerbook}/verify`,
                {
                    method: "POST",
                    headers: {
    ...
    getIceServers(): Promise<IceServers[]> {
        return new Promise((resolve, reject) => {
            ...
            this.pb.session
                .sendCTRLMsg(new ControlMessage("ice_servers"))
