...
export class Shell {
    ...
    lineAboveForm = 0;
    reconnectForm = [{ prompt: "Reconnect" }, { prompt: "Close" }];
    ...
    handleTab() {
        ...
        for (const c of this.commands) {
            if (c[0].startsWith(cmd)) matches.push(c[0]);
        }
        if (matches.length == 1) {
    ...
    async handleLine(input: string) {
        ...
        if (input) this.history.unshift(input);
        this.clearPrompt();
    }
    ...
    async execute(cmd: string, args: string[]) {
        ...
        for (const c of this.commands) {
            if (c[0].startsWith(cmd))
                if (exec == null) exec = c[1].execute;
                else return this.t.writeln(`Ambiguous command: ${cmd}`);
        }

        if (exec == null)
            return this.t.writeln(
                `Command not found: "${cmd}" (hint: \`help\`)`,
            );
        this.active = false;
        try {
            await exec(args);
    ...
    async runForm(
        ...
        this.map.showLog(true);
        this.lineAboveForm =
            this.t.buffer.active.baseY + this.t.buffer.active.cursorY;
        this.t.write("\r\x1B[K");
        this.t.scrollToBottom();
        if (title) this.t.writeln(title);
    ...
    async paste() {
        ...
        if (cb.type != "text/plain") return;
        const text = cb.value;
        if (this.activeForm) {
            this.activeForm.field += text;
            if (!this.activeForm.hidden) this.t.write(text);
        } else if (this.active) {
            this.t.write(text);
    ...
    async keyHandler(key: string) {
        ...
        if (key == "Escape") {
            await this.escape();
        } else if (form?.onKey) form.onKey(key);
        else if (this.active) this.onKey(key);
    }
    ...
    printBelowForm(text: string, returnToForm = false) {
        if (!this.activeForm) return;
        this.t.write(
            `\x1B[s\x1B[${this.activeForm.fields.length - this.activeForm.currentField}B\n\x1B[K${text}`,
        );
        if (returnToForm) this.t.write(`\x1B[u`);
    }
    ...
    updateCapsLock(ev: KeyboardEvent) {
        ...
        if (ev.key == "CapsLock")
            // if the key is CapsLock the state is not updated yet
            capsOn = !capsOn;
        const e = document.getElementById("capslock-indicator");
        if (capsOn) e.classList.remove("hidden");
    ...
    startWatchdog({
        timeout,
        prefix = "Connecting",
    }: { timeout?: number; prefix?: string } = {}): Promise<void> {
        if (!timeout) timeout = terminal7.conf.net.timeout;
        // only one watchdog, the last one
        if (this.watchdog) this.stopWatchdog();
    ...
    startHourglass({
        ...
        this.timer = window.setInterval(() => {
            ...
            this.t.write(
                `\r\x1B[K${prefix}: ${" ".repeat(i)}ᗧ${"·".repeat(dots)}🍒\x1B[?25l`,
            );
            i++;
        }, interval);
    }
    ...
    getGate(prefix: string) {
        ...
        if (maybes.length > 1) {
            ...
            this.t.write(
                `Multiple gates found with prefix ${prefix}: ${maybes.map((g) => g.name).join(", ")}`,
            );
            return null;
        }
        return maybes[0];
    ...
    async onUnauthorized(gate: Gate) {
        ...
        const suffix = Capacitor.isNativePlatform()
  ...
  Copy to clipboard${suffix}`,
        ...
        if (ans == "y") {
            ...
            this.t.writeln(
                "Next, paste the command in a legacy terminal and reconnect",
            );
            let res: string;
            try {
    ...
    async offerInstall(gate, firstOption?): Promise<boolean> {
        ...
        if (gate.fp && !gate.online) {
            ...
            this.t.writeln(
                `Please run \x1B[1mwebexec start\x1B[0m on the server`,
            );
        } else {
            this.t.writeln(
    ...
    async offerSub(gate): Promise<boolean> {
        this.t.writeln(
            "\rJoin our subscribers for persistent sessions and WebRTC 🍯",
        );
        const reconnect = [
        ...
        } else if (res == "Close Gate") gate.close();
        else gate.focus();
