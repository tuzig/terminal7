...
export class Window {
    ...
    open(e) {
        ...
        interact(this.nameE)
            ...
            .on("hold", (ev) => {
                ...
                if (this.gate.activeW != this) return;
                this.rename();
                ev.preventDefault();
                ev.stopPropagation();
            });
    ...
    focus() {
        ...
        if (last) {
            ...
            last.e.classList.add("invisible");
        }
    ...
    close() {
        ...
        if (this.activeP && this.activeP.zoomed) this.activeP.unzoom();
        this.gate.windows.splice(this.gate.windows.indexOf(this), 1);
        this.gate.goBack();
    }
    fit() {
        if (this.rootLayout) this.rootLayout.fit();
    }
    moveFocus(where) {
        ...
        switch (where) {
            case "left":
                match = (p) =>
                    Math.abs(p.xoff + p.sx - a.xoff) < 0.00001 &&
                    p.yoff <= y &&
                    p.yoff + p.sy >= y;
                break;
            case "right":
                match = (p) =>
                    Math.abs(a.xoff + a.sx - p.xoff) < 0.00001 &&
                    p.yoff <= y &&
                    p.yoff + p.sy >= y;
                break;
            case "up":
                match = (p) =>
                    Math.abs(p.yoff + p.sy - a.yoff) < 0.00001 &&
                    p.xoff <= x &&
                    p.xoff + p.sx >= x;
                break;
            case "down":
                match = (p) =>
                    Math.abs(a.yoff + a.sy - p.yoff) < 0.00001 &&
                    p.xoff <= x &&
                    p.xoff + p.sx >= x;
                break;
        }
        this.t7.cells.forEach((c) => {
            if (nextPane == null && c instanceof Pane && c.w && c.w == this)
                if (match(c)) nextPane = c;
        });
        if (nextPane) {
            nextPane.focus();
            this.gate.sendState();
    ...
    updateDivideButtons() {
        ...
        if (this.activeP.isSplittable("topbottom")) bV.classList.remove("off");
        else bV.classList.add("off");
        if (this.activeP.isSplittable("rightleft")) bH.classList.remove("off");
        else bH.classList.add("off");
    }
    syncLayout(thatLayout, theseCells = null) {
        if (!theseCells) theseCells = this.rootLayout.allCells();
        thatLayout.w = this;
        ...
        thatLayout.cells.forEach((thatCell) => {
            ...
            if (thatCell.dir) {
                ...
                const thisI = theseCells.findIndex(
                    (c) => c.channelID == thatCell.channelID,
                );
                if (thisI >= 0) {
