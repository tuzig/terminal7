...
export class SSHChannel extends BaseChannel {
    ...
    handleData(m) {
        if (m instanceof Object && "data" in m) this.onMessage(m.data);
        else {
            this.t7.log("ssh read got error ", m);
            if (this.onClose && (!terminal7.autoReconnect || m.error != "EOF"))
...
export class SSHSession extends BaseSession {
        ...
        return new Promise((resolve, reject) => {
            ...
            SSH.startSessionByKey({
                ...
                .catch((e) => {
                    ...
                    else this.onStateChange("failed", Failure.Aborted);
                    reject(e);
                });
        });
    }
    passConnect(marker?: Marker, password?: string) {
        ...
        SSH.startSessionByPasswd(args)
            ...
            .catch((e) => {
                ...
                else this.onStateChange("failed", Failure.FailedToConnect);
    ...
    openChannel(
        ...
        return new Promise((resolve, reject) => {
            ...
            SSH.newChannel({ session: this.id }).then(({ id }) => {
                console.log("got new channel with id ", id);
                channel.id = id;
                SSH.startShell({ channel: id, command: cmd as string }, (m) =>
                    channel.handleData(m),
                )
                    .then((callbackID) => {
    ...
    async startCommand(cmd: string, onData) {
        ...
        channel.id = id;
        try {
            await SSH.startShell({ channel: channel.id, command: cmd }, (m) =>
                onData(channel.id, m),
            );
        } catch (e) {
...
export class HybridSession extends SSHSession {
        ...
        message.data
            ...
            .forEach(async (line) => {
                ...
                if ((c as { candidate? }).candidate)
                    ...
                    try {
                        await this.webrtcSession.pc.setRemoteDescription(
                            c as RTCSessionDescriptionInit,
                        );
                    } catch (e) {
                        this.t7.log(
                            "got error setting remote desc:",
                            e.message,
                            c,
                        );
                    }
    ...
    async openChannel(
        ...
        if (!this.webrtcSession) {
            ...
            return this.webrtcSession.openChannel(
                cmd,
                parent,
                sx,
                sy,
    ...
    async reconnect(marker?: Marker, publicKey?: string, privateKey?: string) {
        ...
        else return this.connect(marker, privateKey, publicKey);
    }
    close() {
        if (this.webrtcSession) return this.webrtcSession.close();
    }

    getPayload(): Promise<string> {
        if (this.webrtcSession) return this.webrtcSession.getPayload();
        else return super.getPayload();
    }
    setPayload(payload: string): Promise<string> {
        if (this.webrtcSession) return this.webrtcSession.setPayload(payload);
        else return super.setPayload(payload);
    }
    disconnect(): Promise<number | null> {
        if (this.webrtcSession) return this.webrtcSession.disconnect();
