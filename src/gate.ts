...
export class Gate {
    ...
    open(e) {
        ...
        if (t) {
            ...
            t.querySelector(".add-tab").addEventListener("click", () =>
                this.newTab(),
            );
    ...
    private parseServerPayload(
        rawPayload: string | null,
    ): ServerPayload | null {
        let layout: ServerPayload | null = null;
        try {
            if (rawPayload) layout = JSON.parse(rawPayload);
        } catch (e) {
            layout = null;
        }
        return layout;
    ...
    private async applyServerPayload(
        rawPayload: string | null,
    ): Promise<boolean> {
        ...
        if (hadSession) {
            if (!incomingSession || incomingSession !== this.sessionId) {
                ...
                if (layout) layout.session = this.sessionId;
                this.clear();
                needsPersist = true;
            }
        } else {
        ...
        if (freshSession) this.notify("fresh webexec session");
    ...
    private async shouldRestoreSession(
        session: Session,
        publicKey?: string,
        privateKey?: string,
    ): Promise<boolean> {
        if (session.isSSH) return true;
    ...
    onSessionState(state: string, failure: Failure) {
        if (!this.session) {
            this.t7.log(
                `Ignoring ${this.name} change state to ${state} as session is closed`,
            );
            return;
        }
    ...
    async handleSSHFailure() {
        ...
        try {
            toConnect = terminal7.pb.isOpen()
                ? await shell.offerInstall(this, "I'm feeling lucky")
    ...
    async handleFailure(failure: Failure) {
        ...
        switch (failure) {
            ...
            case Failure.NotSupported:
                ...
                } else this.notify("WebRTC agent unreachable");
                break;
            ...
            case Failure.CertificateExpired:
                this.notify(
                    "Security certificate expired. Generating new certificate...",
                );
                terminal7
                    .generateCertificate()
                    .then(() => {
                        terminal7.log(
                            "Certificate regenerated after failure, reconnecting",
                        );
                        this.connect().catch((e) => {
                            terminal7.log(
                                "Failed to reconnect after certificate renewal",
                                e,
                            );
                        });
                    })
                    .catch((e) => {
                        terminal7.log(
                            "Failed to regenerate certificate after failure",
                            e,
                        );
                        this.notify(
            ...
            case Failure.FailedToConnect:
                const firstGate =
                    (await Preferences.get({ key: "first_gate" })).value ===
                    null;
        ...
        if (res == "Reconnect") {
            shell
                .startWatchdog()
                .catch(() => this.handleFailure(Failure.TimedOut));
            try {
                await this.connect();
            } catch (e) {
    ...
    reconnect(): Promise<void> {
        return new Promise((resolve, reject) => {
            ...
            if (++this.reconnectCount > terminal7.conf.net.retries) {
                this.notify(
                    `Reconnect failed after ${this.reconnectCount} attempts`,
                );
                this.reconnectCount = 0;
            ...
            if (session == null) {
                ...
                if (this.tryPB) this.connect().then(resolve).catch(reject);
                else this.handleFailure(Failure.NotSupported);
                return;
            }

            const finish = async (layout) => {
                await this.applyServerPayload(
                    typeof layout == "string" ? layout : null,
                );
                this.reconnectCount = 0;
                resolve();
            };
            ...
            this.t7
                ...
                .then(({ publicKey, privateKey }) => {
                    this.shouldRestoreSession(session, publicKey, privateKey)
                        .then((restore) => {
                            ...
                            session
                                .reconnect(this.marker, publicKey, privateKey)
                                .then(finish)
                                .catch((e) => {
                                    if (this.session != session)
                                        // session changed, ignore the failure
                ...
                .catch((e) => {
                    ...
                    this.t7.log(
                        "reconnect failed, calling the shell to handle it",
                        this.wasSSH,
                        e,
                    );
                    this.t7.notify(e);
    ...
    connect(): Promise<void> {
        return new Promise((resolve, reject) => {
            ...
            this.onConnected = () => {
                this.notify(
                    `🥂  over ${this.session.isSSH ? "SSH" : "WebRTC"}`,
                    true,
                );
                this.wasSSH = this.session.isSSH;
    ...
    panes(): Pane[] {
        ...
        this.t7.cells.forEach((c) => {
            if (c instanceof Pane && c.gate == this) r.push(c);
        });
        return r;
    }
    ...
    setLayout(state: ServerPayload | null = null) {
        ...
        if (
            state == null ||
            !(state.windows instanceof Array) ||
        ...
        if (!this.activeW) this.activeW = this.windows[0];
    ...
    scaleContainer(width?, height?) {
        const container = this.e.querySelector(
            ".windows-container",
        ) as HTMLDivElement;
        if (this.fitScreen) {
    ...
    clear() {
        ...
        if (this.activeW?.activeP?.zoomed) this.activeW.activeP.unzoom();
        this.windows = [];
        this.breadcrumbs = [];
        this.t7.cells.forEach((c, i, cells) => {
            if (c instanceof Pane && c.gate == this) cells.splice(i, 1);
        });
    }
    /*
    ...
    dump(): ServerPayload {
        ...
        this.windows.forEach((w) => {
            ...
            if (w == this.activeW) win.active = true;
            windows.push(win);
        });
        const container = this.e.querySelector(
            ".windows-container",
    ...
    goBack() {
        ...
        if (this.windows.length > 0) {
            if (this.breadcrumbs.length > 0) this.breadcrumbs.pop().focus();
            else this.windows[0].focus();
        }
    }
    fit() {
    ...
    async completeConnect(): Promise<void> {
        ...
        this.session.onStateChange = (state, failure?) =>
            this.onSessionState(state, failure);
        this.session.onCMD = (msg: ControlMessage) => {
            ...
            switch (msg.type) {
                ...
                case "set_clipboard":
                    ...
                    else Clipboard.write({ image: msg.args["data"] });
                    session.sendCTRLMsg(
                        new ControlMessage("ack", { ref: msg.message_id }),
                    );
                    break;
                default:
        ...
        if (this.session.isSSH) {
            ...
            const firstGate = (await Preferences.get({ key: "first_gate" }))
                .value;
            if (firstGate) terminal7.ignoreAppEvents = true;
    ...
    async updateNameE() {
        ...
        if (!e || e.children.length < 1) return;
        this.map.update({
            e: e,
    ...
    close() {
        ...
        setTimeout(() => {
            ...
            if (this == terminal7.activeG) {
                if (this.activeW) this.t7.goHome();
