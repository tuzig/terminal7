...
async function connectCMD(shell: Shell, args: string[]) {
    ...
    if (overPB && !gate.verified) {
        try {
            await terminal7.pb.verifyFP(
                gate.fp,
                "Unverified peer. Please enter OTP to verify or CTRL-C",
            );
        } catch (e) {
            if (native) {
    ...
    const v = await Preferences.get({ key: "first_gate" });
    if (v.value != "nope")
        setTimeout(() => {
            console.log("Toggling help", v.value);
...
async function addCMD(shell: Shell) {
    const f = [{ prompt: "Enter destination (ip or domain[:port])" }];
...
async function resetCMD(shell: Shell, args: string[]) {
        ...
        switch (choice) {
            ...
            case "Reset connection":
                ...
                try {
                    ...
                    shell.t.writeln(
                        "Failed to connect. Please try again and if it keeps failing, close and connect fresh.",
                    );
                    shell.t.writeln(
                        "  Please take the time to write your flow\n  in ##🪳bugs🪳at https://discord.com/invite/rDBj8k4tUE",
                    );
                }
                return;
    ...
    switch (res) {
        ...
        case "Fingerprint":
            ...
            shell.t.writeln(
                "Cleared fingerprint and disconnected from PeerBook",
            );
            terminal7.pbClose();
...
async function editCMD(shell: Shell, args: string[]) {
    const hostname = args[0];
    if (!hostname) return shell.t.writeln("Missing hostname");
    const gate = shell.getGate(hostname);
    if (!gate) return shell.t.writeln(`Host not found: ${hostname}`);
    const isPB = !!gate.fp;
    const fMain = [
        { prompt: "Edit" },
        { prompt: "Connect" },
    ...
    let fFields = [
        ...
        { prompt: "Username", default: gate.username || "" },
        {
            ...
            validator: (a) => {
                ...
                return isNaN(port) || port > 65535 || port < 1
                    ? "Port must be a 16 bit number"
                    : "";
            },
        },
        {
            prompt: "SSH only",
            values: ["y", "n"],
            default: gate.onlySSH ? "y" : "n",
        },
    ];
    const fDel = [
        {
            prompt: isPB
                ? `Delete ${gate.name} from PeerBook?`
                : `Delete ${gate.name}?`,
            values: ["y", "n"],
            default: "n",
        },
    ...
    if (isPB) {
        ...
        shell.t.writeln(
            `You can also edit this peer in the web interface at \n${url}`,
        );
    }
    ...
    switch (choice) {
        case "Connect":
            ...
            break;
        case "Edit":
            enabled = await shell.runForm(
                fFields,
                "choice",
                `\x1B[4m${gate.name}\x1B[0m edit`,
            );
            if (!enabled) {
                await gate.t7.clear();
                return;
...
async function gatesCMD(shell: Shell) {
    ...
    const truncate = (s: string) =>
        ...
        hostAtAddr = (g: Gate) =>
            g.addr ? `${g.username || "TBD"}@${g.addr}` : "",
        fp = (g: Gate) => (g.fp ? `${g.fp.slice(0, 4)}…${g.fp.slice(-4)}` : "");
    const attrs = terminal7.gates.map((g) => [
        truncate(g.name),
        truncate(hostAtAddr(g)),
        fp(g),
    ]);
    const maxLengths = attrs.reduce(
        (a, b) => [Math.max(a[0], b[0].length), Math.max(a[1], b[1].length)],
        [0, 0],
    );

    let res =
        "\x1B[1m" +
        "Name".padEnd(maxLengths[0] + 2) +
    ...
    res +=
        attrs
            .map(
                (a) =>
                    a[0].padEnd(maxLengths[0] + 2) +
                    a[1].padEnd(maxLengths[1] + 2) +
                    a[2],
            )
            .join("\n") + "\n";
...
async function closeCMD(shell: Shell, args: string[]) {
    ...
    if (args[0]) {
        gate = shell.getGate(args[0]);
        if (!gate) return shell.t.writeln(`Host not found: ${args[0]}`);
    } else {
        gate = terminal7.activeG;
        if (!gate) return shell.t.writeln("No active connection");
    }
    gate.close();
}
...
async function subscribeCMD(shell: Shell) {
    ...
    if (
        ...
        !customerInfo.entitlements.active.peerbook
    ) {
        ...
        shell.t.write(
            "\t\t\t(\x1B]8;;https://terminal7.dev/privacy\x07Privacy Policy\x1B]8;;\x07 & ",
        );
        shell.t.writeln(
            "\x1B]8;;https://www.apple.com/legal/internet-services/itunes/dev/stdeula/\x07Terms of Service\x1B]8;;\x07)",
        );
        const TYPES = {
            MONTHLY: "Month",
            TWO_MONTH: "2 Months",
        ...
        const packages = offer.availablePackages.map((p) => {
            ...
            if (introPrice) {
                const price =
                        introPrice.price == 0 ? "Free" : introPrice.priceString,
                    unit = introPrice.periodUnit.toLowerCase(),
                    period =
                        introPrice.periodNumberOfUnits == 1
                            ? unit
                            : `${introPrice.periodNumberOfUnits} ${unit}s`;
        ...
        if (choice == "Restore Purchases") {
            ...
            try {
                ...
                shell.stopWatchdog();
                shell.t.writeln(
                    "Error restoring purchases, please try again or `support`",
                );
                return;
            }
            shell.stopWatchdog();
            ...
            try {
                ...
                shell.stopWatchdog();
                shell.t.writeln(
                    "Error purchasing, please try again or `support`",
                );
                return;
            } finally {
    ...
    if (!terminal7.pb.isOpen()) {
        ...
        try {
            await terminal7.pb.connect({
                token: customerInfo.originalAppUserId,
            });
        } catch (e) {
            if (e == "Unregistered") {
                ...
                shell.t.writeln(
                    "Failed to connect to PeerBook, please try again or `support`",
                );
                return;
            } else {
    ...
    const answer = await shell.askValue(
        `Copy user id to the clipboard? (y/N)`,
        "n",
    );
...
export async function installCMD(shell: Shell, args: string[]) {
    ...
    if (!native) {
        if (!terminal7.pb?.isOpen()) {
            shell.t.writeln(
                "If you are subscribed to PeerBook please `login` first",
            );
            const res = await shell.runForm(
                [
                    { prompt: "Just install" },
            ...
            if (res == "Login") await loginCMD(shell);
        }
    ...
    if (uid) {
        ...
        if (host != "api.peerbook.io") cmd += ` PEERBOOK_HOST=${host}`;
        cmd += " \\\n";
    }
    ...
    let done = false;
    let error = false;
    const fields: Fields = [{ prompt: "Copy command" }, { prompt: "Cancel" }];
    if (native) fields.unshift({ prompt: "Connect & send command" });
    ...
    if (choice == "Copy command") {
        ...
        if (res == "Reconnect") await gate.connect();
        else {
            gate.close();
            shell.map.showLog(false);
        }
    ...
    session.onStateChange = async (state, failure?: Failure) => {
        ...
        switch (state) {
            ...
            case "connected":
                ...
                document.getElementById("log").style.borderColor =
                    "var(--remote-border)";
                channel.onClose = () => {
                    ...
                    if (match) {
                        ...
                        setTimeout(async () => {
                            ...
                            try {
                                await terminal7.pb.verifyFP(
                                    fp,
                                    "Finished install, enter OTP to verify",
                                );
                            } catch (e) {
    ...
    while (!done && !error) await new Promise((r) => setTimeout(r, 100));
    if (done) {
        // wait for the gate to get an fp
        let timedOut = false;
        shell
            .startWatchdog({ timeout: terminal7.conf.net.timeout })
            .catch(() => (timedOut = true));
        while (!gate.fp && !timedOut)
            await new Promise((r) => setTimeout(r, 100));
        if (gate.fp)
            shell.t.writeln(
                `Gate is installed & verified, use \`connect ${gate.name}\``,
            );
        else shell.t.writeln("Install failed, please try again or `support`");
        shell.stopWatchdog();
    } else {
...
async function configCMD(shell: Shell) {
    ...
    shell.t.writeln(
        "https://github.com/tuzig/terminal7/wiki/Setting-file-format",
    );
...
async function supportCMD(shell: Shell) {
    ...
    const choice = await shell.runForm(
        fieldsNative,
        "menu",
        "Choose an option",
    );
    ...
    switch (choice) {
        ...
        case "Send log to support":
            ...
            let email =
                terminal7.conf.peerbook.email ||
                (await shell.askValue("Enter your email address"));
            while (
            ...
            let description = await shell.askValue(
                "Please explain how to recreate the issue: \n",
            );
            description += `\n\nOn: ${navigator.userAgent}\n`;
            shell.t.writeln("Sending...");
            shell.startWatchdog({ timeout: 5000 }).catch(() => sendFailed());
            const res = await fetch(
                `${schema}://${terminal7.conf.net.peerbook}/support`,
                {
                    method: "POST",
                    body: JSON.stringify({
                        email,
...
async function loginCMD(shell: Shell) {
    ...
    if (customerInfo.entitlements.active.peerbook) {
        shell.t.writeln(
            "You are already subscribed, please `subscribe` to login",
        );
        return;
    }
