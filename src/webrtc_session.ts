...
export class WebRTCChannel extends BaseChannel {
    ...
    constructor(session: WebRTCSession, id: number, dc: RTCDataChannel) {
        super();
        this.id = id;
    ...
    get readyState(): string {
        if (this.dataChannel) return this.dataChannel.readyState;
    ...
    resize(sx: number, sy: number): Promise<string> {
        return this.session.sendCTRLMsg(
            new ControlMessage("resize", { pane_id: this.id, sx, sy }),
        );
    }
    close(): void {
...
export class WebRTCSession extends BaseSession {
    ...
    onIceCandidate(e: RTCPeerConnectionIceEvent): void {
        throw new Error("Unimplemented method onIceCandidate()");
    }
    // eslint-disable-next-line
    onNegotiationNeeded(ev: Event): void {
        throw new Error("Unimplemented method onNegotiationNeeded()");
    }
    public get isSSH() {
        return false;
    }
    ...
    async connect(
        ...
        this.pc.onconnectionstatechange = () => {
            ...
            if (state === "failed") this.closeChannels();
            if (state !== "closed" && (state !== "connected" || marker == null))
        ...
        if (marker != null) {
            this.sendCTRLMsg(new ControlMessage("restore", { marker }))
                ...
                .catch((e) => {
                    ...
                    if (e != Failure.TimedOut) this.onStateChange("failed", e);
                });
        }
    }
    isOpen(): boolean {
    ...
    openChannel(
        ...
        return new Promise((resolve, reject) => {
            ...
            if (sx !== undefined) {
                if (typeof cmdorid === "string") cmdorid = [cmdorid];
            ...
            this.pendingChannels[msg.message_id] = (
                dc: RTCDataChannel,
                id: ChannelID,
            ) => {
    ...
    async reconnect(
        ...
        if (marker != null) {
            let payload: string;
            try {
                payload = await this.sendCTRLMsg(
                    new ControlMessage("restore", { marker }),
                );
            } catch (e) {
                if (e != Failure.TimedOut) throw e;
            }
            return payload;
            ...
            cdc.onmessage = (m) => {
                ...
                if (msg.type == "ack" || msg.type == "nack") {
                    ...
                    if (msg.type == "nack") {
                        ...
                        else terminal7.log("A nack is unhandled", msg);
                    } else {
                        if (handlers && typeof handlers.ack == "function")
                            handlers.ack(msg.args.body);
                        else terminal7.log("an ack is unhandled", msg);
                    }
                } else if (this.onCMD) this.onCMD(msg);
            };
        });
    }
    sendCTRLMsg(msg: ControlMessage): Promise<string> {
        return new Promise((resolve, reject) => {
            ...
            if (msg.message_id === undefined) msg.message_id = this.lastMsgId++;
    ...
    setPayload(payload: string | object): Promise<string> {
        return this.sendCTRLMsg(
            new ControlMessage("set_payload", { Payload: payload }),
        );
    }
    closeChannels(): void {
...
export class PeerbookSession extends WebRTCSession {
    ...
    async onNegotiationNeeded() {
        ...
        try {
            await this.t7.pb.adminCommand(
                new ControlMessage("offer", { target: this.fp, sdp: d }),
            );
        } catch (e) {
            terminal7.log("failed to send offer", e);
            // ensure it's not an old session
            if (this.pc != null) this.onStateChange("failed", e);
            return;
        }
    ...
    async peerAnswer(offer) {
        ...
        if (this.pc.signalingState == "stable") {
            terminal7.log(
                "got an answer but signla're stable, ignoring answer",
            );
            return;
        }
...
export class HTTPWebRTCSession extends WebRTCSession {
    ...
    constructor(address: string, headers?: Map<string, string>) {
        ...
        if (headers) headers.forEach((v, k) => (this.headers[k] = v));
        terminal7.log(
            "new http webrtc session",
            address,
    ...
    sendOffer(offer: RTCSessionDescriptionInit) {
        ...
        this._fetch(this.address, "POST", offer.sdp)
            .then((response) => {
                if (response.status == 401) this.fail(Failure.Unauthorized);
                else if (response.status >= 300) {
                    terminal7.log("failed to post to PB", response);
                    if (response.status == 404) this.fail(Failure.NotSupported);
                    else this.fail();
            ...
            .then((data) => {
                ...
                if (this.pc)
                    this.pc
                        .setRemoteDescription({ type: "answer", sdp: data })
                        .catch(() => {
                            // on some machines (android), we need a newline at the end the answer
                            terminal7.log(
                                "failed to set remote description, trying with a newline",
                            );
                            data += "\n";
                            this.pc
            ...
            .catch((error) => {
                terminal7.log(
                    `FAILED: POST to ${this.address} with ${JSON.stringify(this.headers)}`,
                    error,
                );
                ...
                // Check for certificate-related errors (often manifest as network/TLS errors)
                    ...
                    terminal7.log(
                        "Detected certificate-related error",
                        error.message,
                    );
                    this.fail(Failure.CertificateExpired);
                } else this.fail(Failure.NotSupported);
            });
    }
    ...
    sendCandidate(candidate: RTCIceCandidate) {
        ...
        this._fetch(
            ...
            .catch((error) => {
                terminal7.log(
                    `FAILED: PATCH to ${this.address} with ${JSON.stringify(this.headers)}`,
                    error,
                );
                if (error.message == "unauthorized")
                    this.fail(Failure.Unauthorized);
