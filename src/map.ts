...
export class T7Map {
        ...
        return new Promise((resolve) => {
            ...
            openEmulator(e, this.t0).finally(() => {
                ...
                if (Capacitor.getPlatform() === "android") {
                    ...
                    this.t0.element.addEventListener(
                        "input",
                        (ev: Event & { data? }) => {
                            if (ev.data) this.shell.keyHandler(ev.data);
                        },
                    );
                }
                this.shell.start();
            ...
            if (log) {
                ...
                document
                    .getElementById("log-minimized")
                    .addEventListener("click", (ev) => {
                        this.showLog(true);
                        ev.stopPropagation();
                        ev.preventDefault();
                    });
                interact(document.getElementById("map")).on("tap", (ev) => {
                    this.showLog(false);
    ...
    add(g: Gate): Element {
        ...
        d.gate = g;
        if (g.fp) d.classList.add("from-peerbook");
        ...
        interact(pad)
            ...
            .on("tap", (ev) => {
                ...
                if (ev.dt < 500) {
                    ev.stopPropagation();
                    const command = ev.target.closest(".gate-edit")
                        ? "edit"
                        : "connect";
                    this.shell
                        .runCommand(command, [g.name])
    ...
    refresh() {
        ...
        document
            .querySelectorAll("#gates .empty-pad")
            .forEach((e) => e.remove());
        ...
        terminal7.gates.forEach(async (g: Gate) => {
            ...
            if (g && g.session && (g.session as WebRTCSession).getStats) {
                ...
                const getBytes = (bytes: number) => {
                    ...
                    if (i >= sizes.length) return "1TB+";
                    return +(bytes / Math.pow(1024, i)).toFixed(2) + sizes[i];
                };
                const pad = (s: string, n = 9) =>
                    s.padEnd(n, "X").replace(/X/g, "&nbsp;");
                const extraClass =
                    stats.roundTripTime > 400
                        ? "error"
                ...
                onGate =
                    `<i class="f7-icons ${extraClass}">arrow_up_arrow_down_circle</i><span class=${extraClass}>` +
                    pad(stats.roundTripTime + "ms", 7) +
                    "</span>";
                onMap =
                    onGate +
                    '<i class="f7-icons">arrow_down_circle</i>' +
                    pad(getBytes(stats.bytesReceived)) +
                    '<i class="f7-icons">arrow_up_circle</i>' +
                    pad(getBytes(stats.bytesSent));
            }
            ...
            if (terminal7.activeG === g) {
                ...
                else e.classList.remove("zoomed");
            }
        });
    }
    /*
    ...
    showLog(show = undefined) {
        ...
        */
        if (show) {
            minimized.classList.add("invisible");
            log.classList.remove("hidden");
            document.getElementById("log-button").classList.add("on");
            this.t0.focus();
        } else {
            minimized.classList.remove("invisible");
    ...
    close() {
        if (this.updateTask != 0) clearInterval(this.updateTask);
    }
}
