...
let checkedC = 0;
export function resetTWR() {
    checkedC = 0;
}
export async function reloadPage(page) {
    console.log("-- Reloading Page --");
    await page.reload({ waitUntil: "commit" });
    await page.waitForFunction(() => window.terminal7 !== undefined);
    await page.evaluate(() => {
...
export async function connectFirstGate(page, gateName?) {
    ...
    if (!gateName) btn = btns.first();
    else btn = btns.filter({ hasText: gateName }).first();
    await btn.click();
}
...
export async function getLines(page, start = -1, end = -1): Array<string> {
    return await page.evaluate(
        ({ start, end }) => {
            const buffer =
                window.terminal7.activeG.activeW.activeP.t.buffer.active;
...
export async function waitForTWROutput(
    page,
    ...
    throw new Error(
        `Timeout waiting for TWR output. Expected: ${expected}, got: ${buffer}`,
    );
}
