...
test.describe("terminal7 UI", () => {
    ...
    test.beforeAll(async ({ browser }) => {
        ...
        await page.evaluate(async () => {
            window.terminal7.notify = (msg: string) =>
                console.log("NOTIFY: " + msg);
            localStorage.setItem(
                "CapacitorStorage.dotfile",
                `
[net]
...
insecure=true`,
    ...
    test("a gate ssh port can be edited", async () => {
        ...
        port = await page.evaluate(async () => {
            while (
                !window.terminal7 ||
                !window.terminal7.gates ||
                window.terminal7.gates.length === 0
            )
    ...
    test("a host with no port can added", async () => {
        ...
        const port = await page.evaluate(async () => {
            while (
                !window.terminal7 ||
                !window.terminal7.gates ||
                window.terminal7.gates.length < 2
            ) {
                await new Promise((r) => setTimeout(r, 100));
            }
    ...
    test("a host with a port can added", async () => {
        ...
        const port = await page.evaluate(async () => {
            while (
                !window.terminal7 ||
                !window.terminal7.gates ||
                window.terminal7.gates.length < 3
            ) {
                await new Promise((r) => setTimeout(r, 100));
            }
    ...
    test("gates can be deleteted", async () => {
        ...
        await page.keyboard.press("Enter");
        await sleep(100);
        const numOfGates = await page.evaluate(
            async () => terminal7.gates.length,
        );
    ...
    test("connect to gate with no webexec, get install command", async () => {
        ...
        */
        ...
        await runSSHCommand(
            webexecSSHConfig,
            "bash -c 'WEBEXEC_SERVER_URL=http://webexec:7777 webexec start'",
        );
        await sleep(3000);
    });
    ...
    test("a pane can be reseted", async () => {
        ...
        await page.evaluate(async () => {
            ...
            const origNotify = window.terminal7.notify.bind(window.terminal7);
            window.terminal7.notify = (m, dontShow) => {
                window.notifications.push(m);
                origNotify(m, dontShow);
            };
            // ensure help bubbles won't auto-show on connect
            localStorage.setItem("CapacitorStorage.first_gate", "nope");
        });
        await connectFirstGate(page, "foo");
        await sleep(1000);
        const helpBubbles = page.locator(".help-bubble-text");
        const count = await helpBubbles.count();
        for (let i = 0; i < count; i++)
            await expect(helpBubbles.nth(i)).toBeHidden();
        ...
        // wait for reconnection to complete and notification to fire
        await page.waitForFunction(
            () => window.notifications.some((n) => /over WebRTC/.test(n)),
            { timeout: 5000 },
        );
        const notifications = await page.evaluate(() =>
            window.notifications.map((n) => n.replace(/\x1B\[[0-9;]*m/g, "")),
        );
        expect(
            notifications.some((n) => /foo:.* over WebRTC/.test(n)),
        ).toBeTruthy();
    });
    test("how a gate handles disconnect", async () => {
        await page.evaluate(
            async () => (window.terminal7.conf.net.timeout = 1000),
        );
        await runSSHCommand(
            {
                host: "webexec",
