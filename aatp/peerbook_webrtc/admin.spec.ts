...
const url = process.env.LOCALDEV
    ? "http://localhost:3000"
    : "http://terminal7:8080";
...
test.describe("peerbook administration", () => {
    ...
    let redisClient: redis.Redis, page: Page, context: BrowserContext;
    ...
    test("purchase update with an active subscription and bad otp", async () => {
        ...
        await page.evaluate(async () => {
            ...
            try {
                ...
                if (e == "Unregistered") terminal7.pb.register();
            }
        });
        await sleep(2500);
    ...
    test("local and peerbook gates are properly displayed", async () => {
        ...
        await page.evaluate(() => {
            localStorage.setItem(
                ...
                JSON.stringify([{ id: 0, addr: "webexec", name: "foo" }]),
            );
        });
        await reloadPage(page);
    ...
    test("rename a peer", async () => {
        ...
        const gateName = await page
            .locator('[data-test="gate-name"]')
            .first()
            .innerText();
    ...
    test("check email, click url and ensure client is logged in", async ({
        request,
        browser,
    }) => {
        await sleep(200);
    ...
    test("test the support command", async ({ request }) => {
        ...
        await sleep(100);
        await page.evaluate(() => {
            terminal7.log("log line");
        });
        await page.keyboard.press("Enter");
        await waitForTWROutput(page, /address:/, 2000);
        await page.keyboard.type("test@gmail.com");
        await page.keyboard.press("Enter");
        await waitForTWROutput(page, /issue:/, 2000);
